...
def usage():
    ...
    sys.stderr.write(" ## \t -a all_seq_info.TXT.....output info by all seq without windows (see below)\t\t\t\t##\n")
...
def main(): 
    ...
    parser.add_argument("-a","--allinfo", required=False, default="all_seq_info.txt",
        dest="all_seq_info", help="Output text file to save at and gc percent, as well as at and gc skew [default: all_seq_info.txt]")
    ...
    sys.stdout.write("    all info out file: %s\n" % args.all_seq_info) 
    ...
    sys.stdout.flush()
    #Prep output file
    gene_name = os.path.splitext(os.path.basename(args.in_file))[0] ## get the input file name without extension
    o_file = open(args.out_file,'w')
    aio_file = open(args.all_seq_info,'w')
    o_file.write("%s\tIndex\tGC Skew (%ib)\n" % (gene_name, args.window_size))
    aio_file.write("%s\tseq_length\tIndex\tGC_percent\tAT_percent\tGC_Skew\tAT_Skew\n" % (gene_name))
    ...
    for i in range(1,tot+1): 
        ...
        c = 0.0
        curr_seq = ""
        clean_my_seq_len = len(re.sub("[^A-Z]","",my_seq))
        percent_G_C = (my_seq.count("G")+my_seq.count("C"))/clean_my_seq_len*100
        percent_A_T = (my_seq.count("A")+my_seq.count("T"))/clean_my_seq_len*100
        skew_G_C = (float(my_seq.count("G"))-float(my_seq.count("C")))/(float(my_seq.count("G"))+float(my_seq.count("C")))
        skew_A_T = (float(my_seq.count("A"))-float(my_seq.count("T")))/(float(my_seq.count("A"))+float(my_seq.count("T")))
        aio_file.write("%s\t%s\t%i\t%0.2f\t%0.2f\t%0.4f\t%0.4f\n" % (my_description, clean_my_seq_len, i, percent_G_C,percent_A_T,skew_G_C,skew_A_T))
    ...
    dataframe = pandas.read_csv(args.all_seq_info,delimiter="\t")
    #csv_name = args.all_seq_info.replace("txt","csv")
    dataframe.to_csv(args.all_seq_info+".csv", encoding='utf-8', index=False)
